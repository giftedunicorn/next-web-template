...
const Login = () => {
  ...
  const { user, signinWithGoogle, signinWithEmail, signinWithGitHub } = useAuth();
  ...
  const _signinWithEmail = () => {
    ...
    signinWithEmail(email, password, '/dashboard')
      .catch((error) => {
        message.error(error.message)
      })
  }
      ...
      <div className="layout">
        <Card className="container">
          <Row style={{ marginBottom: 32 }} >
            <Col span={11}>
              <Title level={5}>Log in with Email:</Title>
              <Input 
                placeholder="Email"
              ...
              />
              <Button type="primary" size="large" style={{ marginBottom: 32, width: 164 }} onClick={_signinWithEmail}>{t("login")}</Button>
            </Col>
            <Col span={2} style={{ textAlign: "center" }}>
              <Divider type="vertical" style={{ height: "100%" }}/>
            </Col>
            <Col span={11}>
              <Title level={5}>Log in with other provider:</Title>
          ...
          <Title level={5} type="secondary">Need an account? Sign up here.</Title>
          <Title level={5} type="secondary">Forgot your password?</Title>
        </Card>
      </div>
      <AppFooter />
      {renderStyles()}
